...
class Currency
{
    ...
    {
        if ( filter_has_var(INPUT_POST, 'currency_rate') ) {
            $currency_rate = filter_input(INPUT_POST, 'currency_rate', FILTER_SANITIZE_STRING);
            $currency_rate = str_replace(',', '.', $currency_rate);
            $this->prepare_rate($currency_rate);
        }
    }
